...
// Print the current bitmap on the screen
void print() {
        ...
        for (int j = length - 1; j >= 0; j--) {
            ...
            for (int k = 4; k >= 0; k--) {
                if ((E[i][j] >> k) == 1) {
                    ...
                    int leader = 5 * j + k;
                    if (R[leader][j] != 0) {
                        // If there is an elimination row, eliminate it, need to XOR the entire row
                        // Dynamic threads
                        const int num_threads = std::thread::hardware_concurrency();  // Get the available number of threads
