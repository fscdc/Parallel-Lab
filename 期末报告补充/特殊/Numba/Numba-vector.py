...
def xor_vectorized(a, b):
    return a ^ b

# Special Gaussian elimination serial numpy array implementation
def solve():
    start = time.time()

    # Traverse eliminated rows: eliminate element by element, clear once
    for i in range(num_eliminated_rows):
        is_eliminated = False
        ...
        for j in range(length - 1, -1, -1):
            for k in range(4, -1, -1):
                if (E[i][j] >> k) == 1:
                    ...
                    leader = 5 * j + k
                    if R[leader][j] != 0:
                        # If there is an elimination element, eliminate it, need to XOR the entire row
                        E[i, :j+1] = xor_vectorized(E[i, :j+1], R[leader, :j+1])
                    else:
                        # Otherwise, lift, after lifting, this entire row can be ignored
